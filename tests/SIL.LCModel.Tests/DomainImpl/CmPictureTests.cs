	...
	public class CmPictureTests: MemoryOnlyBackendProviderRestoredForEachTestTestBase
	{
		...
		public void CmPictureConstructor_FromTextRep_MissingFilename()
		{
			Assert.That(m_pictureFactory.Create("CmPicture||||||This is a caption||",
				CmFolderTags.LocalPictures),
				Throws.ArgumentException.With.Message.Match("File path not specified.(\r)?\nParameter name: srcFile"));
		}
		...
		{
			Assert.That(m_pictureFactory.Create("CmPicture||whatever.jpg||||This is a caption||", CmFolderTags.LocalPictures),
				Throws.ArgumentException.With.Message.Match("File does not have a rooted pathname: whatever.jpg(\r)?\nParameter name: srcFile"));
		}
